...
- `eigen`: the covariance matrix's eigenvalues and eigenvectors, in the same form as the output of _eigen()_. This means that the argument passed should be a list with an entry containing values and another entry containing vectors. Further, the eigenvalues should be ordered from greatest to least.
...
By default, reasonable upper and lower limits for the x-axis and y-axis are chosen by the function. With `xl` and `yl`, the user can provide limits via vectors.
