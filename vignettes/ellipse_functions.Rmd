...
For the two dimensions under consideration, _confidenceEllipse()_ plots a two-dimensional (1-`alpha`)100% confidence ellipse from a p-dimensional multivariate normal distribution.
...
- `X.mean` - the mean of the two dimensions of the p-dimensional multivariate normal distribution, as represented by a column matrix
...
The simple example below illustrates basic use of the function for the _ability.cov_ dataset. We begin by finding the neccessary values, and then calling the function.

```{r}
x.bar <- ability.cov$center[5:6] #mean
print(x.bar)
...
The value for `alpha` can be changed from its default of 0.05. Notice the slight change in the resulting ellipse.
...
If at least one of either `x1` or `y1` is set as NULL, one can adjust the limits computed by the function by providing a value for `lim.adj`. Positive values increase the distance and thus make the ellipse appear smaller. Negative values result in the opposite. By default, `lim.adj = 0.02`.
...
Further, by default, `center = FALSE`. If `center = TRUE`, the center of the ellipse will be indicated with a point, and dashed lines are drawn from this point to the x and y axes.
